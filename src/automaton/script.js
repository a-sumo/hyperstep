...
import audioFile1 from "[javascript/auto]!=!!!file-loader!../assets/audio/synth_melody.mp3";
...
function init() {
  ...
  fileInput.addEventListener('click', onPlayClickHandler);
}
...
function startAudioProcessing(stream) {
  ...
  if (gumStream.active) {
    ...
    // file_src = createMediaElementSource(audioFile1);
...
function setupAudioGraph() {
  ...
  // As a workaround, here we create a gain node with zero gain, and connect it to the system audio output.
  if(recording){
